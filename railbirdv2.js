...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('wf-form-reference-code').addEventListener('submit', function(event) {
        ...
        var apiReferenceCode = `${xanoApiBaseUrl}/reference_code`;
        ...
        fetch(apiReferenceCode, {
            ...
            body: JSON.stringify({ email: email, reference_code: referenceCode }),
        })
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
        ...
        .then(data => {
            // Assuming the API returns the same reference code back
            document.getElementById('uk-reference').innerText = 'Your reference code is: ' + data.reference_code; 
        })
        .catch(error => {
