...
function calculateAveragePercentage() {
  fetch(`${xanoApiBaseUrl}/sports_percentage`)
    ...
    .then(records => {
      // Filter records to only include those with the same sportsId as currentSportsQuestionId
      const filteredRecords = records.filter(record => record.sportsId === currentSportsQuestionId);
      ...
      // Calculate the average based on the filtered records
