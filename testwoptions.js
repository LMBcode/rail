var typed = new Typed(".typing-text", {
  ...
  function getYesOrNoEntertainmentQuestion() {
    fetch(`${xanoApiBaseUrl}/entertainment_q2`)
      .then(response => response.json())
      .then(questions => {
        if (questions.length === 0) {
          throw new Error('No questions available');
        ...
        // Select a random question
        const randomQuestion = questions[Math.floor(Math.random() * questions.length)];
        displayEntertainmentQuestion(randomQuestion);
      })
      .catch(error => console.error('Error:', error));
  }

  //Display EntertainmentQuestion
  function displayEntertainmentQuestion(question) {
    // Set the question text and calculate the yes percentage
    let currentQuestion = document.querySelector('.question_entertainment')
    ...
    //percentageResult.textContent = `${yesPercentage.toFixed(1)}%`;
      //successResult.textContent = successResultText;
    // Select the buttons
    const yesButton = document.querySelector('.button-yes_entertainment');
    const noButton = document.querySelector('.button-no_entertainment');
    
    // Remove any existing event listeners (if applicable) and add new ones
    yesButton.onclick = () => {
        incrementEntertainmentVote(question.id, 'yes_votes');
    }
    noButton.onclick = () => {
        incrementEntertainmentVote(question.id, 'no_votes');
    }
  }

  // Increment Entertainment
  function incrementEntertainmentVote(questionId, voteType) {
    // Get the current vote counts
    let successCard = document.querySelector('.success-message-card-flip_entertainment')
    fetch(`${xanoApiBaseUrl}/entertainment_q2/${questionId}`)
      .then(response => response.json())
      .then(questionData => {
        const updatedQuestionData = {
          ...questionData,
        ...
        .then(() => {
          // After updating, re-fetch and refresh the UI with the new data
                //getQuestionsAndDisplayPercentages();
           successCard.style.display = "block"; // Unhide the card
        })
        .catch(error => console.error('Error updating vote:', error));
      })
      .catch(error => console.error('Error fetching current vote count:', error));
    ...
    while(questionData[`option_${optionsCount + 1}`] !== null) {
    ...
    for (let i = 1; i <= optionsCount; i++) {
      ...
      label.textContent = questionData[`option_${i}`];
     ...
     // optionsContainer.appendChild(itemMovieDiv);
      ...
      submitButton.onclick = function() {
        ...
        selectedOptions.forEach(optionNumber => {
          incrementOptionVote(questionData.id, optionNumber);
        });
      };
