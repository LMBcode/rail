var typed = new Typed(".typing-text", {
  ...
  function noVoteHandler(question) {
    ...
    function displayQuestionWithOptions(questionData) {
      ...
      const optionsContainer = document.querySelector('.form_checkbox_field_wrapper');
    ...
    let totalVotes = 0;
    const optionCount = Object.keys(questionData).filter(key => key.startsWith('option_') && key.endsWith('_text')).length;
    ...
    for (let i = 1; i <= optionCount; i++) {
        const optionText = questionData[`option_${i}`];
        if (!optionText) {
            console.error(`Option text for option_${i} not found`);
      ...
      */
      ...
      checkbox.className = 'form_checkbox-option';
      checkbox.id = `option_${i}`; // Unique ID for each checkbox
      checkbox.value = `option_${i}`; // Set the value to the option key
  ...
  */
  ...
  const submitButton = document.getElementById('entertainment-submit');
  submitButton.addEventListener('click', function() {
      const selectedOptions = document.querySelectorAll('input.form_checkbox-option:checked');
      ...
      selectedOptions.forEach(optionInput => {
          // This will get '1', '2', etc., based on which checkbox is selected
          const selectedOptionNumber = optionInput.id.split('_')[2];
          incrementOptionVote(questionData.id, selectedOptionNumber);
      });
  });
